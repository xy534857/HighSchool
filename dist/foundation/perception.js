...
import {personalClock} from './timing.js';

export function clockInfo(world,owner){return personalClock(world,owner);}
...
export function view(world,owner){
  ...
  return {actor,actors,contacts:contacts(world,owner,actors),relationships:Object.fromEntries(Object.keys(s.actors).filter(id=>id!==owner).map(id=>[id,relationship(world,owner,id)])),knownByKey,objects,objectsById:Object.fromEntries(objects.map(o=>[o.id,o])),offers,invitations,groups,messages,situations:personalSituations(world,owner),obligations:personalObligations(world,owner),session:session?{id:session.id,host:session.host,topic:session.topic,created:session.created,access:session.access,active:activeMembers(s,session),floor:clone(session.floor)}:null,clock:clockInfo(world,owner),known};
 }
