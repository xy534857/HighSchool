...
本轮增加跨人物关系谋划、经历引发的有限人格变化、每日反思队列、异步模型接口及人工接管调试。LLM 返回声明式知识，经校验编译为 Soar productions 或写入原生个人目标；NPC 继续选择和执行行动。情景提供机会与冲突，不能指定他人的接受或结局。
...
这是独立的三人实时调试实验：只设置一次公开排斥事件，之后放开自治。操作目录是 `.runtime-lab/`，需要操作者逐次读取并回答六份真实请求。为容纳人工操作，该实验的回复截止为 180 秒；真实等待、行动轨迹和安装耗时写入 `research/runtime/`。它不是无人值守测试，也不代表模型供应商延迟。普通自动化回归使用 `npm test`。
